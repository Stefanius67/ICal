...
class iCalEventReader extends Reader
{
    ...
    /** @var boolean    currently reading inside of a VALARM property     */
    protected bool $bInAlarm = false;
    ...
    public function hasEndReached(string $strLine) : bool
    {
        ...
        if ($bEnd) {
            ...
            if ($this->oEvent->hasRecurrentEvents()) {
    ...
    protected function parseDtStart(string $strName, string $strValue, array $aParams) : void
    {
        ...
        if ($uxtsStart !== null) {
            ...
            if (isset($aParams['TZID'])) {
                ...
                if ($oTimezone !== null) {
                    $this->oEvent->setCalcTimezone($oTimezone);
                } else {
    ...
    protected function parseDuration(string $strName, string $strValue, array $aParams) : void
    {
        ...
        $iDuration = $this->parseDurationValue($strValue, $this->oEvent->getAllDay());
        if ($iDuration !== null) {
    ...
    protected function beginAlarmProp(string $strName, string $strValue, array $aParams) : void
    {
        if ($strValue == 'VALARM') {
            $this->bInAlarm = true;
            $this->oICalendar->log(LogLevel::WARNING, 'Not supportet VALARM found');
        }
    }

    /**
     * End current VALARM property.
     * @param string $strValue
     * @param array<string,string> $aParams
     */
    protected function endAlarmProp(string $strName, string $strValue, array $aParams) : void
    {
        if ($strValue == 'VALARM') {
            $this->bInAlarm = false;
        }
